...
///// ARRUMAR VALOR FINAL NO WHATSAPP ////////
  ...
  Finalizar.addEventListener('click',function(){
    ...
    let Lanche = Arr.map(UserChoice => {
        return `🍔 Produto: ${UserChoice.FoodName},%0A📝 Descrição: ${UserChoice.Description},%0A💲 Preço(UNI) : ${UserChoice.Price},%0A🔢 Quantidade: ${UserChoice.Quantity},%0A💵 Valor: ${UserChoice.Total.toFixed(2)} %0A %0A`;
      });
      console.log(Lanche)
   ...
   // function enviarWhatsApp(Finalizar) {
       ...
       //   const textoCompleto = `Olá Usuario, Informações adicionais:. Segue o pedido:\n\n${Resultado}`;
         ...
         //   const urlEncoded = encodeURIComponent(textoCompleto);
            ...
            const urlWhatsApp = `https://wa.me/${5511965404572}?text=${Resultado +  `*______________________________*  %0A 💰 VALOR TOTAL: ${Total.toFixed(2)} %0A %0A *Nós da SANTO´S FOOD agradecemos pela preferência.* 🧡 `}`;
